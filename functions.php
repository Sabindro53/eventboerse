...
function eb_serve_theme_root_file() {
    ...
    if ( preg_match( '#^/(assets|audit)/([A-Za-z0-9][A-Za-z0-9._-]*\.json)$#', $path, $m ) ) {
        ...
        // Dieselbe Frage wie fuer die Seite selbst — sonst laedt das HQ fuer
        // einen berechtigten Besucher eine leere Huelle, oder schlimmer: die
...
add_action( 'wp_head', function() {
    // Nur Favicon. SEO-Meta (og/twitter/title) wird per-page in index.php gesetzt (#4c) —
    // hier KEINE generischen og/twitter-Tags mehr, sonst Duplikate, die (weil später im
    // <head>) die korrekten Per-Page-Tags überschreiben.
...
function eventboerse_setup() {
    // 'title-tag' bewusst NICHT aktiviert (#4c): index.php setzt den <title> per-page;
    // WP soll keinen konkurrierenden generischen <title> via wp_head erzeugen.
...
function eb_hq_proxy_darf() {
    return eb_hq_zugang_offen();
}

/**
 * Verwaltung: strenger als das HQ selbst.
 *
 * `/hq/mitarbeiter` erteilt und entzieht Zugaenge. Das darf der
 * Generalzugang NICHT — ein geteiltes Passwort, mit dem man weitere
 * Zugaenge vergeben kann, ist nicht mehr einzufangen. Hier gilt weiter:
 * angemeldeter Administrator, sonst nichts.
 */
function eb_hq_verwaltung_darf() {
...
add_action( 'rest_api_init', function () {
    register_rest_route( 'eventboerse/v1', '/hq/mitarbeiter', array(
        ...
        // Bewusst strenger als die uebrigen HQ-Routen: hier werden Zugaenge
        // vergeben, und der Generalzugang darf das nicht.
