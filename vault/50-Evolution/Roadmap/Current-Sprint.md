...
- **Playwright-Suite: 1124 Tests in 74 Suiten**, blockierendes Gate in `pr-check.yml`.
...
| **Die Hochzeit-Bausteine schließen den Kreis nicht** — `js/modules/board/43-planungs-zentrale.js` (#268) bietet 18 Fragmente, jedes mit Kategorie und einem Knopf „*Kategorie* finden". Der Knopf öffnet die Anbieter-Auswahl vorgefiltert — und merkt sich **nicht**, für welchen Baustein. `fragment.cardId` wird im ganzen Frontend an **genau einer** Stelle geschrieben: von Hand über ein Dropdown (Zeile 211). Wer über den Baustein sucht und bucht, hat danach eine Karte auf dem Board und einen Baustein, der weiter „noch offen" zeigt — und dessen Budget in der Restsumme (Zeile 165) weiter mitgerechnet wird | Den ausstehenden Baustein beim Öffnen der Auswahl merken und beim Anlegen der Karte verknüpfen; beim Abbrechen wieder vergessen, sonst bekommt der nächste Zugang eine falsche Verknüpfung. Gemessen wird die **Wirkung** (`cardId` gesetzt, Restbudget sinkt), nicht das Markup | claude |
