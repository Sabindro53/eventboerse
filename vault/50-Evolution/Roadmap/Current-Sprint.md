...
- **Playwright-Suite: 1146 Tests in 77 Suiten**, blockierendes Gate in `pr-check.yml`.
...
|---|---|---|
| ~~**Der Provisionssatz stand elfmal im Frontend**~~ — **behoben am 14.09.2026.** `eb_stripe_platform_fee_rate()` liest ihn aus `wp-config.php` und schickt ihn als `application_fee_amount` an Stripe; das Frontend trug ihn als **drei** feste Zahlen und **acht** ausgeschriebene „3%" nach — darunter der Abrechnungsbeleg aus `downloadBusinessInvoice()`, zwei Nachrichten an den Dienstleister und die Buchungserklärung in der Shell. Ein gesetzter Satz hätte alle acht in derselben Sekunde falsch gemacht, ohne Fehlermeldung | Die drei Werte kommen über `eventboerseApi.gebuehren`, werden gegen dieselben Grenzen geprüft wie in PHP, und der Text wird aus dem Satz gerechnet (`ebProvisionText()`). 7 Tests, 7 Mutationen | — |
| ~~**`renderSidebarUpcoming()` erfindet drei Termine**~~ — **behoben am 14.09.2026.** — `js/modules/board/42-guide-social-feed.js:1710` liefert eine fest verdrahtete Liste („Rock Festival Berlin, 12. Sep 2026", „Hochzeitsmesse Köln", „Oktoberfest Opening") unter der Überschrift *Demnächst*. Am 14.09.2026 gemessen: die Funktion wird gerufen, `#sidebarUpcoming` steht in der Shell — und der erste Eintrag liegt bereits **in der Vergangenheit**. Eine Liste, die mit der Zeit unwahr wird, ohne dass jemand etwas ändert | Entweder aus dem Aktivitäten-Bestand speisen (`assets/eb-aktivitaeten.json` trägt Termine mit Datum und Gebiet) oder den Block weglassen. Sie speist sich jetzt aus `assets/eb-aktivitaeten.json` und steht in `search/14-aktivitaeten.js`, wo der Bestand liegt — ein zweiter Lader wäre eine zweite Wahrheit. Vergangenes und Abgesagtes wirft `ebAktivitaetenImUmkreis()` ohnehin heraus; die drei leeren Fälle sagen Verschiedenes, und die alte Stelle trägt jetzt den Befund samt Warnung, dass eine zweite Definition in der Verkettung gewinnen würde. 7 Tests, 5 Mutationen | — |
| **Das Ruleset auf `main` erzwingt den falschen Job** — am 14.09.2026 über die API gemessen: genau ein Pflicht-Kontext, `PR Check / PR-Validierung (pull_request)`. Das ist der Job `check` (PHP-Syntax + zwei Kommentare, 9 s). Die **16 Tore und 1124 Tests** liegen in `tests` / `E2E-Testsuite (Playwright)` und sind **nicht** erzwungen. Ein PR mit roter Suite war mergefähig, sobald PHP parst. Verdeckt war das, solange der Kontext gar nicht traf — Astras richtige Namensreparatur in #268 hat aus fail-closed ein fail-open gemacht | **Im Code behoben** (`needs: tests` + `if: always()` + Torschritt, der bei nicht-`success` mit 1 aussteigt; ohne `always()` würde der Job übersprungen, und ein übersprungener Pflicht-Check gilt als bestanden). 5 Mutationen, Verhalten ausgeführt statt gelesen. **Offen und nur vom Inhaber zu machen:** das Ruleset zusätzlich auf `E2E-Testsuite (Playwright)` stellen — der Code-Weg ersetzt die Einstellung nicht, er trägt sie nur, solange sie aussteht | **Inhaber** (Ruleset) |
| ~~**Die Hochzeit-Bausteine schließen den Kreis nicht**~~ — **behoben am 14.09.2026.** — `js/modules/board/43-planungs-zentrale.js` (#268) bietet 18 Fragmente, jedes mit Kategorie und einem Knopf „*Kategorie* finden". Der Knopf öffnet die Anbieter-Auswahl vorgefiltert — und merkt sich **nicht**, für welchen Baustein. `fragment.cardId` wird im ganzen Frontend an **genau einer** Stelle geschrieben: von Hand über ein Dropdown (Zeile 211). Wer über den Baustein sucht und bucht, hat danach eine Karte auf dem Board und einen Baustein, der weiter „noch offen" zeigt — und dessen Budget in der Restsumme (Zeile 165) weiter mitgerechnet wird | Den ausstehenden Baustein beim Öffnen der Auswahl merken und beim Anlegen der Karte verknüpfen; beim Abbrechen wieder vergessen, sonst bekommt der nächste Zugang eine falsche Verknüpfung. Die Herkunft steht jetzt **am offenen Dialog** (`data-planning-fragment`), nicht in einer Modulvariablen — sie stirbt mit ihm, ein Abbruch kann also keine spätere, fremde Karte an einen Baustein hängen. Die Projekt-Kennung wird mitgeprüft (beide Hochzeiten tragen dieselben Baustein-Kennungen), und die Liste wird festgeschrieben, bevor hineingeschrieben wird (sonst landet die Verknüpfung in einer Ableitung). 7 Tests im echten Browser, 5 Mutationen. **Astras Arbeit war gut verdrahtet** — zwölf Funktionen, keine ohne Aufrufer; es fehlte allein das letzte Glied | — |
