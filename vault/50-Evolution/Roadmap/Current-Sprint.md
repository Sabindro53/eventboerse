...
| `⚡ HQ-Puls` scheitert seit 1675 (jetzt 1675, 1676, 1677, **1678** vom 13.09. 11:03). Ursache im Log gefunden: `agent.mjs --check` bricht mit *„Verbotsmuster im Journal: E-Mail-Adresse (×2)"* ab. Journal 400/396/4. Regex `(?!kontakt@)[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}` in `scripts/lib/verbotsmuster.mjs:41` trifft eine echte Modell-Ausgabe (Lina Okafor schrieb u. a. *„@eb.de sind temporär blockiert"*), die per SFTP in `assets/eb-arbeit.json` landet | Zwei Wege, Inhaber entscheidet: (a) im Modell-Systemauftrag Beispiele ohne `@` verlangen und die zwei betroffenen Einträge einmalig aus dem Live-Journal löschen; (b) den Filter in `agent.mjs` VOR dem Schreiben ansetzen statt danach nur zu melden — dann würde die Rolle sauber `abgebrochen` statt der ganze Puls rot. Vier Läufe in Folge rot ist genau die Sorte, die nach Woche 3 nicht mehr gelesen wird | Inhaber |
| `assets/eb-arbeit.json` trägt zwei E-Mail-Muster in älteren Modellantworten. Der SFTP-Abruf lädt sie bei jedem Lauf des Puls neu, weshalb ein Bugfix ohne Bereinigung der Live-Datei den Puls nicht wieder grün macht | Nach der Wahl oben: entweder Live-Datei per SFTP einmalig neu schreiben (nur mit sauberem Journal) oder Filter greift vor dem Schreiben, dann die zwei Alt-Einträge einzeln entfernen | Inhaber |
