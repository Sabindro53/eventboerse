...
- [x] **Verfügbarkeit + Sofortbuchung komplett (Audit-Pass 2026-06-16)**: Inserate haben jetzt Blackout-Termine (Urlaub/vergeben) zusätzlich zu Wochentagen, DB-Spalte unavailable_dates via dbDelta migriert (db_version 2.1). Sofortbuchung beim Käufer mit echtem Datepicker (Validierung gegen Whitelist+Blackouts), Uhrzeit, Veranstaltungsort, Gästezahl und Sonderwünsche als Pflicht-/Optional-Felder. Buchungs-Details fließen durch info-Objekt → Stripe-Metadata → Board-Karte → Auftragsboard mit strukturierten Tiles.
- [x] **Fee-Transparenz**: Live-Preview im Inseraterstellungs-Formular zeigt sofort beim Preis-Eingeben was Dienstleister bekommt. Settings hat neue „Auszahlungen & Gebühren"-Karte mit 3 Tiles + 2 Rechenbeispielen + Stripe-Dashboard-Link. Detail-Seite booking-note dynamisch je nach Sofortbuchung-Modus.
- [x] **B2B-Buchungen**: Sofortbuchung hat optionalen „Geschäftliche Buchung"-Toggle → klappt Firma/Rechnungsadresse/USt-IdNr auf. Auto-Aktivierung wenn currentUser als Firma registriert ist (sub_role=business oder company gesetzt). B2B-Block erscheint im Auftragsboard prominent (grüner Border, eigene Card).
- [x] **Release-Checkliste**: `docs/release-checklist.md` — 10 Schritte vom Merge bis Launch, inkl. Stripe-Webhook-Setup, Live-Mode-Switch, Smoke-Test-Skript, PWA-Verifikation, DB-Backup-Anleitung.
