...
| `barrierefreiheit.spec.js` | axe-core WCAG 2.0/2.1/**2.2** AA, beide Farbmodi × **32 der 34 Seiten**, angemeldet und mit `isAdmin`; Seitenliste aus den `id="page-…"` der Shell **abgeleitet**; Gegenprobe auf die wirklich aktive Seite; Fokus; Dot-Labels; jede Formularbeschriftung nennt ihr Feld | **0 Verstöße** (Stand 24.09.2026 nachgemessen, 10/10 Tests). Die zwei übrigen Seiten sind nachgewiesene Weiterleitungen (`home` → `browse`, `profile` → `provider`), deren Ziel selbst gemessen wird. Hier stand bis zum 24.09. „deckt 5 von 34 Seiten … 40 Knoten, 20 `critical`" — das galt vor der Weitung am 15.09. und war seither eine Warnung ohne Gegenstand |
